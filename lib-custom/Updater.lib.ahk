Check_ForUpdate(_ReplaceCurrentScript = 0, _SuppressMsgBox = 0, _CallbackFunction = "", ByRef _Information = "", _AskToInstall = 1)
{

  /*
	Version.ini file format
	
  http://www.autohotkey.net/~Rseding91/Self%20Script%20Updater/Version.ini
  ...
  ;Msgbox, % Script_Name ", " Version_Number ", " Update_URL ", " Retry_Count
	...
	, Temp_FileName := A_Temp . "\" . Filler . ".exe"
	, VBS_FileName := A_Temp . "\" . Filler . ".vbs"
  
  ;Msgbox, %Filler%, %Version_File%, %Temp_FileName%, %VBS_FileName%
    ...
    {
      Msgbox,,Error,Couldn't check for updates
      return
    }
    ...
    ;Msgbox, %Version%
		...
		If (Version > Version_Number){
			If (_SuppressMsgBox != 1 and _SuppressMsgBox != 3){
				...
				IfMsgBox,Yes
        {
          MsgBox,0,This may take some time,Please wait until finished downloading`n`nThis instance will be paused to prevent mouse and keyboard misbehaviour during update
    ...
    Else
    {
      ...
      Msgbox, 0, This is up to date!, %_Information%
      return
    }
  ...
  if _AskToInstall and FileExist(Temp_FileName)
  {
  
    Msgbox, 0x4, Ready to see some improvements?, New version downloaded!`n`nDo you want to install it now?
    ifMsgbox Yes
    {
      Run, %Temp_FileName%
      ExitApp
