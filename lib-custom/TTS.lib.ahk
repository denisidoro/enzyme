...
Say(text, voice, method="SpeakWait")
{

  global ClearMemoryAfterSpeaking, DefaultVoice, DefaultVoiceName
  
  if !DefaultVoice
  {
    if voice =
      voice := DefaultVoiceName
    DefaultVoice := TTS_CreateVoice(voice)
  }
    
  TTS(DefaultVoice, method, text)
  
  if ClearMemoryAfterSpeaking > 0
    SetTimer, ClearTTSMemory, % ClearMemoryAfterSpeaking*-1000

}

/*
ChangeDefaultVoice(voice="Microsoft Anna")
{
