/*
;===================================================================================
Listview Supplementary Library v1.1
;===================================================================================
	Functions:
	- LVG_Search()
	- LVG_Get()
...
Parameters:		
	...
	Mode: 
		All					- will search in all rows according to the conditions of RowsToSearchIn
		Selected			- will search only in selected rows according to the conditions of RowsToSearchIn
...
Examples:
	...
	Say you'd like to do the same, with the only exception, that you'd like to know the total number of rows that meet these conditions:
...
LVG_Search(Gui_nr=1,mode="Selected",mode2="Count",rows="all",cols="all",srch_str="") {
	...
	,DeselectedUnchecked,SelectedUnchecked,DeselectedChecked,All
	...
	if (cols <> "all")
	{
		StringSplit, col_a, cols, `; ;Splitting col parameter into subparameters
		Loop, %col_a0%
		{
			c_col_a := a_index 
			...
			{
				start := col_b1
				Loop, % col_b2-col_b1+1 ;+1 so that it includes the last number too (eg. by cols=5-16 it includes 16 too)
				{
					col_c%start% := 1
					start++
				}
